...
describe('Liked Posts', ()=>{
    it('saved posts should be displayed on the page', ()=>{
        render(<LikedPosts/>);
        expect(screen.getAllByTestId('liked-posts')).toBeInTheDocument;
    })
    
    it('saved posts should be taken from the store (selector)', ()=>{
        useSelectorMock.mockReturnValue(posts)
        render(<LikedPosts/>);
        expect(screen.getAllByTestId('liked-posts').length).toBe(posts.length);
    })
})